...
# def suma_AP(d,n):	
	...
	if ( (d-1) % (BASE) == 0):		
		...
		print(f"\n\t'd' value {d} -> Process: {pid} | name: {name}\n\t{FR_GREEN}====== SUM: {place_comma(resultado)} ======{NO_COLOR}")
...
if __name__ == '__main__':
	...
	if N_CPUS > multiprocessing.cpu_count():	
		print(f"\n\tThis PC doesn't have {N_CPUS} CPU's, then we assume max of PC: {multiprocessing.cpu_count()}\n")
		N_CPUS = multiprocessing.cpu_count()

	# Max Number of CPU's 
	print(f"\n{FR_YELL}\t======= Max number of cpu's in PC: {multiprocessing.cpu_count()} ======={NO_COLOR}\n")
