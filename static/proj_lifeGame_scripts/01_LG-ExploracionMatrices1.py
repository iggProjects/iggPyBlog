...
if __name__ == "__main__":
	...
	try:
		...
		mostrar_matriz1(matriz,msg)

		m0 = matriz[ 0:int(nX/2), 0:int(nY/2) ]
		mostrar_matriz1(m0, " m0 = matriz[ 0:int(nX/2), 0:int(nY/2) ]")

		m1 = matriz[ int(nX/2):nX, 0:int(nY/2) ]
		mostrar_matriz1(m1, " m1 = matriz[ int(nX/2):nX, 0:int(nY/2) ]")

		m2 = matriz[ 0:int(nX/2), int(nY/2):nY ]
		mostrar_matriz1(m2," m2 = matriz[ 0:int(nX/2), int(nY/2):nY ]")

		m3 = matriz[ int(nX/2):nX, int(nY/2):nY ]
		mostrar_matriz1(m3," m3 = matriz[ int(nX/2):nX, int(nY/2):nY ]")

		der = m1[ [0], : ]
		mostrar_matriz1(der, " derecha = m1[ [0], : ]")

		izq = m1[ [int(nX/2)-1], : ]
		mostrar_matriz1(izq, " izquierda = m1[ [int(nX/2)-1], : ]")

		m0e = np.vstack( (izq, m0, der) )
		mostrar_matriz1(m0e, " m0 expandida fase 1 = np.vstack( (izq, m0, der) )")

		bot = np.vstack(
			( [m3[int(nX/2)-1, 0]], m2[ :, [0] ], [m3[0, 0]] )
		)
		mostrar_matriz1(bot, " bot = np.vstack( ( [m3[int(nX/2)-1, 0]], m2[ :, [0] ], [m3[0, 0]] ) )")

		top = np.vstack(
			( [m3[int(nX/2)-1, int(nY/2)-1]], m2[ :, [int(nY/2)-1] ], [m3[0, int(nY/2)-1]] )
		)
		mostrar_matriz1(top, " top = np.vstack( ( [m3[int(nX/2)-1, int(nY/2)-1]], m2[ :, [int(nY/2)-1] ], [m3[0, int(nY/2)-1]] ) )")

		m0e = np.hstack( ( top, m0e, bot) )
		mostrar_matriz1(m0e, " m0e expandida = np.hstack( ( top, m0e, bot) )")
