...
def result_script_exec():
    ...
    try:         
        ...
        try: 
            ...
            # call subprocess to excecute py_script_path to get text 
            if opSys == "Windows":
            ...
            # format tyext in lines into csv file
            try:
                ...
                write_log_file("my_messages.txt","ERROR FROM 'func result_script_exec() - subprocess.run, SEE server_messages.txt'")
            ...
            write_log_file("my_messages.txt","ERROR FROM 'func result_script_exec() - getting csv file, SEE server_messages.txt'")
            logging.exception("error from func 'result_script_exec()' when getting csv file ! | exception() => "  + str(Argument))
            error_msg = "error from function 'result_script_exec()' when getting csv file ! => "  + str(Argument)
            return render_template('error_page.html', error_msg = error_msg )
...
def result_script_exec_workers():
    ...
    try:
        ...
        if opSys == "Windows":
            ...
            # put mysite/ in path for "pythonanywhere"
            text = subprocess.run(["/usr/bin/bash", "-c", f"python {basedir}/{py_script_path} '{workers}'"],capture_output=True)
            #text = subprocess.run(["/usr/bin/bash", "-c", f"python mysite/{py_script_path}"],capture_output=True)
        else:
            print(f"Please check how to pass list of parameters for operating system: {opSys}")
        ...
        try:
            ...
            # Create csv file with lines of text from 'list_color_text' 
            # delete if exists  
...
def result_script_html():
    ...
    try:     
        ...
        write_log_file("my_messages.txt","IN 'func result_script_html()'")
